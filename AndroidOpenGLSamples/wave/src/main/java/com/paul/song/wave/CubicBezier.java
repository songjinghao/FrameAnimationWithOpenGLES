...
import com.paul.song.common.interfaces.Renderable;
...
public class CubicBezier implements Renderable {
    ...
    public CubicBezier(BezierRenderer renderer, FloatBuffer vboBuffer,
        ...
        GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, vboBuffer.capacity() * Constant.BYTES_PER_FLOAT,
    ...
    public void render(boolean flipY) {
        ...
        GLES20.glUniform4f(mRenderer.muColorHandle, color[0], color[1], color[2], color[3]);

        GLES20.glUniform4f(mRenderer.muBzDataHandle, starEndPoints[0], starEndPoints[1] * mulY, starEndPoints[2], starEndPoints[3] * mulY);
        GLES20.glUniform4f(mRenderer.muBzDataCtrlHandle, controlPoints[0], controlPoints[1] * mulY, controlPoints[2], controlPoints[3] * mulY);

        final int stride = Constant.BYTES_PER_FLOAT * Constant.T_DATA_SIZE;

        GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, mBufferId);
        GLES20.glEnableVertexAttribArray(mRenderer.maTDataHandle);
        GLES20.glVertexAttribPointer(mRenderer.maTDataHandle,
                Constant.T_DATA_SIZE,
                GLES20.GL_FLOAT,
                false,
                stride,
                0);
        ...
        // Draw it
        GLES20.glDrawArrays(GLES20.GL_TRIANGLES, 0, mRenderer.numberOfPoints * Constant.POINTS_PER_TRIANGLE);
    }

...
public class VerticesDataGenerator implements Runnable {
    ...
    private float[] genTData() {
        ...
        //  | /
        //  3
        float[] tData = new float[Constant.POINTS_PER_TRIANGLE * Constant.T_DATA_SIZE * mBezierRenderer.numberOfPoints];

        float step = 1f / (float)tData.length * 2f;

        for (int i = 0; i < tData.length; i += Constant.POINTS_PER_TRIANGLE) {
